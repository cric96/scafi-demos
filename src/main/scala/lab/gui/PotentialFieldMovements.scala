...
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.{MetaActionProducer, ScafiSimulationExecutor}
import it.unibo.scafi.space.Point3D

case class PotentialFieldMovements() extends MetaActionProducer[(Double, Double)] {
  ...
  override def apply(id: Int, dt: (Double, Double)): MetaAction = if (dt != (0.0, 0.0)) {
    ScafiSimulationExecutor.contract.simulation.get.NodeDtMovement(id, dt)
  } else {

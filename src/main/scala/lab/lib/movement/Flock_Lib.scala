...
trait Flock_Lib {
  ...
  trait FlockLib {
    ...
    def withSeparation(velocity: Velocity)(separationDistance: Double) : P = withSeparation(selector = true)(velocity)(separationDistance)

    /* steer towards the average heading of local flockmates */
    private[Flock_Lib] def alignment(flockingSensor: => Boolean, velocity: Velocity, neighbourCount : Int): Velocity = {
      val averageVelocity: P = getValuesFromActiveNode(flockingSensor)(velocity).fold(Velocity.Zero)(_ + _) / neighbourCount
    ...
    private[Flock_Lib] def cohesion(neighbors: Seq[Point3D]): Velocity = if(neighbors.isEmpty) {
      ...
      (centroid - currentPosition()).normalized
    }
    ...
    private[Flock_Lib] def separation(activeNode : Seq[Point3D], separationDistance: Double): Velocity = {
      ...
      val separationVector = vectorsDirectToNeighbours.fold[P](Point3D.Zero)((acc, b) => acc + b)
